...
test('world generation is deterministic for the same world seed', () => {
  ...
  const spec = { name: 'Asterra', genre: 'fantasy' as const, tone: 'beautiful and grounded', seed: world.metadata.id };
...
test('generated world contains the fixed map hierarchy, social graph and initial economy', () => {
  const world = createKernel('account-a', 1_000).state;
  const generated = generateWorld(world.metadata.id, { name: 'Asterra', genre: 'fantasy', tone: 'grounded', seed: world.metadata.id }, new Date(2_000));
  ...
  assert.equal(generated.npcs.every((npc) => npc.familyNpcIds.length > 0 && npc.friendNpcIds.length > 0), true);
  assert.equal(generated.npcs.some((npc) => npc.enemyNpcIds.length > 0), true);
