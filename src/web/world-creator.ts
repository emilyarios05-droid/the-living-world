...
export function renderWorldCreator(
  ...
  app.innerHTML = `
    <main class="creator-shell">
      <header class="topbar creator-topbar">
        <div><p class="eyebrow">${existingSpec ? 'WORLD SETTINGS' : 'NEW WORLD'}</p><h1>${existingSpec ? 'Shape the world' : 'Build the rules of your reality'}</h1></div>
      ...
      <section class="creator-intro hero-card">
        <p class="eyebrow">WORLD IDENTITY</p>
        <h2>Give the simulation its starting reality.</h2>
        <p>The generator creates the initial geography, settlements, locations, population, economy and fixed map hierarchy from these choices. Later AI systems can evolve the world without replacing its history.</p>
        <div class="world-spec-grid">
          <label class="rule-input"><span>World name</span><input id="world-name" value="${escapeHtml(existingSpec?.name ?? 'A New Living World')}" maxlength="80" /></label>
          <label class="rule-input"><span>Genre</span><select id="world-genre"><option value="fantasy">Fantasy</option><option value="romantasy">Romantasy</option><option value="science-fiction">Science fiction</option><option value="modern">Modern</option><option value="historical">Historical</option><option value="custom">Custom</option></select></label>
          <label class="rule-input"><span>Tone / aesthetic</span><input id="world-tone" value="${escapeHtml(existingSpec?.tone ?? 'beautiful, grounded, immersive, emotionally realistic')}" maxlength="240" /></label>
        </div>
      </section>
      ...
      <section class="creator-actions">
        ...
        <div class="card-actions"><button class="ghost" id="save-draft" type="button">Save draft</button><button class="primary" id="finish-world" type="button">${existingSpec ? 'Save changes' : 'Generate world'}</button></div>
      </section>
    </main>`;
  ...
  const save = async (finalize: boolean) => {
    try {
      ...
      if (finalize && !nextKernel.state.generation) nextKernel = session.withGeneratedWorld(nextKernel, collectSpec());
