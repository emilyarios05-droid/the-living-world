...
export function validateWorld(state: WorldState, constitution?: WorldConstitution): PreflightReport {
  ...
  if (constitution) {
    ...
    for (const rule of constitution.rules) {
      ...
      ids.add(rule.id);
      if (rule.worldId !== state.metadata.id) issues.push({ severity: 'error', code: 'RULE_WORLD_MISMATCH', message: `Rule ${rule.id} belongs to another world.` });
    }
  }
  ...
  if (!generation) {
    ...
    if (!state.simulation) {
      issues.push({ severity: 'error', code: 'SIMULATION_STATE_MISSING', message: 'Generated world has no living simulation state.' });
    } else {
